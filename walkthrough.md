...
## 对话预填体验优化 (2026-06-14 更新)

- **去强控制设计**：检查所有从外部页面（如首页、记录页、洞察页）跳转到对话页时携带预填提示词（prompt/memo/insight）的情形。
- **对话提示横幅 (Suggested Prompt Banner)**：取消了直接向输入框预填的行为，改为在输入框上方渲染非侵入式的智能引导横幅。
- **双向操作支持**：
  - **填入输入框**：点击后将该建议提示词填入输入框，用户可再次修改。
  - **直接发送**：点击后直接使用该提示词向 AI 发起对话，保留原有快捷体验。
  - **关闭横幅**：点击右上角 `X` 忽略此建议。
